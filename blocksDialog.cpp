...
BlocksDialog::BlocksDialog(QString fontFile, QString fontName, QWidget *parent)
  ...
  if (error) {
    QMessageBox::warning(this, "Freetype Not Initialized",
  ...
  if (ftFace_->charmap == nullptr) {
    QMessageBox::warning(this, "Not Unicode",
                         "There is no Unicode charmap in this font!");
    reject();
  }

  for (int n = 0; n < ftFace_->num_charmaps; n++) {
    std::cout << "PlatformId: " << ftFace_->charmap[n].platform_id
              << " EncodingId: " << ftFace_->charmap[n].encoding_id
              << std::endl;
  }
  ...
  std::copy(blocksSet.begin(), blocksSet.end(),
            std::back_inserter(*codePointBlocks_));
  ...
  for (auto block : *codePointBlocks_) {
    ...
    item->setData(Qt::DisplayRole,
                  QString("U+%1").arg(uBlocks[block->blockIdx_].first_, 5, 16,
                                      QChar('0')));
    ...
    item->setTextAlignment(Qt::AlignHCenter);
    item->setData(Qt::DisplayRole,
                  QString("U+%1").arg(int(uBlocks[block->blockIdx_].last_), 5,
                                      16, QChar('0')));
    item->setFlags(item->flags() & ~Qt::ItemIsEditable);
...
void BlocksDialog::checkCreateReady() {
  bool wasDisabled = !ui->createButton->isEnabled();
  ui->createButton->setEnabled(codePointQty_ > 0);
  if (wasDisabled && ui->createButton->isEnabled())
    ui->createButton->setFocus(Qt::OtherFocusReason);
}
...
void BlocksDialog::updateQtyLabel() {
  ui->qtyLabel->setText(
      QString("Number of CodePoints selected: %1").arg(codePointQty_));
}
