      ...
      for (int ch = 0; ch < header->glyphCount; ch++) {
        ...
        if (face->glyphs[ch]->ligKernPgmIndex != 255) {
          ...
          if (lk_idx < header->ligKernStepCount) {
            if ((face->lig_kern_steps[lk_idx]->b.goTo.isAGoTo) &&
                (face->lig_kern_steps[lk_idx]->b.kern.isAKern)) {
              lk_idx = face->lig_kern_steps[lk_idx]->b.goTo.displacement;
            }
            do {
              if (face->lig_kern_steps[lk_idx]->b.kern.isAKern) { // true = kern, false = ligature
                GlyphKernStep *step = new GlyphKernStep;
                step->nextcharCode  = face->lig_kern_steps[lk_idx]->a.nextGlyphCode;
                step->kern          = face->lig_kern_steps[lk_idx]->b.kern.kerningValue;
                glk->kern_steps.push_back(step);
              } else {
                GlyphLigStep *step = new GlyphLigStep;
                step->nextcharCode = face->lig_kern_steps[lk_idx]->a.nextGlyphCode;
                step->charCode     = face->lig_kern_steps[lk_idx]->b.repl.replGlyphCode;
                glk->lig_steps.push_back(step);
              }
            } while (!face->lig_kern_steps[lk_idx++]->a.stop);
          }
        }
...
public:
  IBMFFontMod(uint8_t *memory_font, uint32_t size) : memory(memory_font), memory_length(size) {
  ...
  inline const FaceHeaderPtr get_face_header(int face_idx) { return faces[face_idx]->header; }
  ...
  bool get_glyph_lig_kern(int face_index, int glyph_code, GlyphLigKern **glyph_lig_kern) {
    ...
    *glyph_lig_kern =
        faces[face_index]->glyphs_lig_kern[glyph_code - faces[face_index]->header->firstCode];

    return true;
  }

  bool get_glyph(int face_index, int glyph_code, GlyphInfoPtr &glyph_info, Bitmap **bitmap) {
    if (face_index >= preamble.faceCount) return false;
    if ((glyph_code > faces[face_index]->header->lastCode) ||
        (glyph_code < faces[face_index]->header->firstCode))
      return false;
  ...
  bool save_face_header(int face_index, FaceHeader &face_header) {
  ...
  bool save_glyph(int face_index, int glyph_code, GlyphInfo *new_glyph_info, Bitmap *new_bitmap) {
    if ((face_index < preamble.faceCount) &&
