...
#define WRITE(v, size)                                                                             \
  ...
  auto IBMFFontMod::saveGlyph(int faceIndex, int glyphCode, GlyphInfo *newGlyphInfo, Bitmap *new_bitmap) -> bool {
  ...
  ///
  auto IBMFFontMod::ligKern(int faceIndex, const GlyphCode glyphCode1, GlyphCode *glyphCode2, FIX16 *kern) const -> bool {
  ...
  auto IBMFFontMod::getGlyphLigKern(int faceIndex, int glyphCode, GlyphLigKern **glyphLigKern) const -> bool {
  ...
  auto IBMFFontMod::getGlyph(int faceIndex, int glyphCode, GlyphInfoPtr &glyph_info, Bitmap **bitmap) const -> bool {
    if (faceIndex >= preamble_.faceCount) return false;
    if (glyphCode > faces_[faceIndex]->header->glyphCount) { return false; }
  ...
  auto IBMFFontMod::convertToOneBit(const Bitmap &bitmapHeightBits, Bitmap **bitmapOneBit) ->bool {
  ...
  // similar list of steps is returned, else -1
    auto IBMFFontMod::findList(std::vector<LigKernStep *> &pgm, std::vector<LigKernStep *> &list) const -> int {
    int idx = 0;
    ...
    for (auto &face : faces_) {
      ...
      if (spaceRequired > 0) {
        ...
        for (auto idx = overflowList.rbegin(); idx != overflowList.rend(); idx++) {
          ...
          LigKernStep *ligKernStep = new LigKernStep;
      ...
      for (auto &glyph : face->glyphs) {
        if (glyphsPgmIndexes[glyphIdx] == -1) {
          ...
          if ((abs(glyphsPgmIndexes[glyphIdx]) >= 255) &&
              (abs(glyphsPgmIndexes[glyphIdx]) < 5000)) {
            QMessageBox::warning(nullptr, "Logic Error",
  ...
  auto IBMFFontMod::charSelected(char32_t ch, SelectedBlockIndexesPtr &selectedBlockIndexes) const -> bool {
      ...
      for (uint16_t i = 0; i < kernTableHeader.nTables; i++) {
        ...
        if (error == 0) {
          if (kernSubTableHeader.coverage.data.format == 0) {
            ...
            length = kernSubTableHeader.length -
